...
class RatingService implements RatingServiceInterface
{
    ...
    {
        $ratings = $this->ratingRepository->getProductRatingsCursorPaginated($productId, $perPage, $cursor);
        $hasNextPage = $ratings->count() > $perPage;

        if ($hasNextPage) {
            $ratings->pop();
        }

        $nextCursor = $hasNextPage && $ratings->isNotEmpty() ? $ratings->last()->id : null;
    ...
    {
        $ratings = $this->ratingRepository->getUserRatingsCursorPaginated($userId, $perPage, $cursor);
        $hasNextPage = $ratings->count() > $perPage;
